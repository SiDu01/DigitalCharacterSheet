...
internal static partial class DataQualityReportGenerator
{
    ...
    private static double CalculateParserPriority(IEnumerable<UnhandledCase> cases)
    {
        ...
        var caseTypeScore = items.Max(item => item.CaseType switch
        {
            ...
            "spell-cancellation-effect-candidate" => 58,
            "spell-storage-effect-candidate" => 58,
            "item-autonomous-spell-effect-candidate" => 58,
            "spell-detection-reference-candidate" => 46,
            "spell-recharge-rule-candidate" => 46,
            "spell-consumable-use-candidate" => 46,
            "item-group-member-reference" => 36,
            "spellbook-level-list-candidate" => 57,
            "item-random-spell-effect-candidate" => 58,
            "spell-sensor-effect-candidate" => 46,
            ...
            "item-granted-limited-spell-uses-candidate" => 58,
            "item-action-spell-list-activation-candidate" => 58,
            "item-action-direct-spell-activation-candidate" => 58,
            "item-passive-direct-spell-access-candidate" => 58,
            "item-charge-spell-list-activation-candidate" => 58,
            "item-charge-direct-spell-activation-candidate" => 58,
            "item-triggered-charge-spell-effect-candidate" => 58,
    ...
    private static bool IsAncestryCategory(string category)
    {
        return string.Equals(category, "race", StringComparison.OrdinalIgnoreCase)
            || string.Equals(category, "race-version", StringComparison.OrdinalIgnoreCase);
    }

    private static bool IsItemCategory(string category)
    {
        return string.Equals(category, "item", StringComparison.OrdinalIgnoreCase)
    ...
    private static TextCaseInfo? ClassifySpellText(string category, string text)
    {
        ...
        if (SpellOrItemTableLabelRegex().IsMatch(cleaned))
        {
            return null;
        }

        if (ItemGroupMemberReferenceRegex().IsMatch(cleaned))
        {
            return new TextCaseInfo(
                "item-group-member-reference",
                "wiki-only",
                0.7,
                "Text is an item group member reference rather than a spellcasting rule.",
                "NoMechanicalRuleParser");
        }

        if (SpellbookLevelListRegex().IsMatch(cleaned))
        {
            return new TextCaseInfo(
                "spellbook-level-list-candidate",
                "candidate",
                0.8,
                "Text appears to be a level-prefixed spellbook or item spell list entry.",
                "SpellbookParser");
        }
        ...
        if (IsItemCategory(category) && ItemChargeSpellPreparationRegex().IsMatch(cleaned))
        {
            return new TextCaseInfo(
                "item-charge-spell-preparation-candidate",
                "candidate",
                0.84,
                "Text spends item charges to replace, prepare, or swap spells rather than cast them directly.",
                "ItemChargeSpellPreparationParser");
        }

        if (IsItemCategory(category) && ItemChargeSpellModifierRegex().IsMatch(cleaned))
        {
            return new TextCaseInfo(
                "item-charge-spell-modifier-candidate",
                "candidate",
                0.84,
                "Text spends item charges to modify a spell being cast.",
                "ItemChargeSpellModifierParser");
        }

        if (IsItemCategory(category) && ItemAtWillSpellActivationRegex().IsMatch(cleaned))
        {
            return new TextCaseInfo(
                "item-at-will-spell-activation-candidate",
                "candidate",
                0.82,
                "Text appears to cast a spell from an item at will.",
                "ItemAtWillSpellActivationParser");
        }
        ...
        {
            return new TextCaseInfo(
                "item-stored-spell-activation-candidate",
                "candidate",
                0.82,
                "Text appears to cast a spell stored inside a consumable, gem, tattoo, card, or similar item.",
                "ItemStoredSpellActivationParser");
        }

        if (IsItemCategory(category) && ItemSpellEffectActivationRegex().IsMatch(cleaned))
        {
            return new TextCaseInfo(
                "item-spell-effect-activation-candidate",
                "candidate",
                0.8,
                "Text grants or duplicates a spell effect through item use without phrasing it as direct spellcasting.",
                "ItemSpellEffectActivationParser");
        }

        if (IsItemCategory(category) && ItemGrantedLimitedSpellUsesRegex().IsMatch(cleaned))
        {
            return new TextCaseInfo(
                "item-granted-limited-spell-uses-candidate",
                "candidate",
                0.82,
                "Text grants a limited number of spell casts from an item outcome or item effect.",
                "ItemGrantedLimitedSpellUsesParser");
        }

        if (IsItemCategory(category) && ItemTriggeredChargeSpellEffectRegex().IsMatch(cleaned))
        {
            return new TextCaseInfo(
                "item-triggered-charge-spell-effect-candidate",
                "candidate",
                0.82,
                "Text spends or loses item charges because another spell or event triggers the item.",
                "ItemTriggeredChargeSpellEffectParser");
        }

        if (IsItemCategory(category) && ItemChargeSpellListActivationRegex().IsMatch(cleaned))
        {
            return new TextCaseInfo(
                "item-charge-spell-list-activation-candidate",
                "candidate",
                0.84,
                "Text spends item charges to cast from a fixed spell list or spell table.",
                "ItemChargeSpellListActivationParser");
        }

        if (IsItemCategory(category) && ItemChargeDirectSpellActivationRegex().IsMatch(cleaned))
        {
            return new TextCaseInfo(
                "item-charge-direct-spell-activation-candidate",
                "candidate",
                0.84,
                "Text spends item charges to directly cast a named spell.",
                "ItemChargeDirectSpellActivationParser");
        }

        if (IsItemCategory(category) && ItemActionSpellListActivationRegex().IsMatch(cleaned))
        {
            return new TextCaseInfo(
                "item-action-spell-list-activation-candidate",
                "candidate",
                0.82,
                "Text uses an action to cast one spell from a fixed item spell list.",
                "ItemActionSpellListActivationParser");
        }

        if (IsItemCategory(category) && ItemActionDirectSpellActivationRegex().IsMatch(cleaned))
        {
            return new TextCaseInfo(
                "item-action-direct-spell-activation-candidate",
                "candidate",
                0.82,
                "Text uses an action to directly cast a named spell from an item.",
                "ItemActionDirectSpellActivationParser");
        }

        if (IsItemCategory(category) && ItemPassiveDirectSpellAccessRegex().IsMatch(cleaned))
        {
            return new TextCaseInfo(
                "item-passive-direct-spell-access-candidate",
                "candidate",
                0.8,
                "Text passively allows a direct named spell cast from an item.",
                "ItemPassiveDirectSpellAccessParser");
        }

        if (IsItemCategory(category) && ItemChargeSpellActivationRegex().IsMatch(cleaned))
        {
            return new TextCaseInfo(
        ...
        if (IsItemCategory(category) && ItemActionSpellActivationRegex().IsMatch(cleaned))
        {
            return new TextCaseInfo(
        ...
        if (IsItemCategory(category) && ItemPassiveSpellAccessRegex().IsMatch(cleaned))
        {
            return new TextCaseInfo(
        ...
        if (IsItemCategory(category) && ItemSpellbookListSentenceRegex().IsMatch(cleaned))
        {
            return new TextCaseInfo(
                "item-spellbook-list-candidate",
                "candidate",
                0.8,
                "Text appears to describe spells recorded in an item spellbook or spell-storing book.",
                "ItemSpellbookListParser");
        }

        if (IsItemCategory(category) && ItemSpellListSentenceRegex().IsMatch(cleaned))
        {
            return new TextCaseInfo(
                "item-spell-list-candidate",
                "candidate",
                0.8,
                "Text appears to describe a fixed spell list provided by an item.",
                "ItemSpellListParser");
        }
        ...
        {
            return new TextCaseInfo(
                "item-spell-stat-block-candidate",
                "candidate",
                0.78,
                "Text appears to provide fixed spell attack or save DC values for an item-cast spell.",
                "ItemSpellStatBlockParser");
        }

        if (SpellComponentSubstitutionRegex().IsMatch(cleaned))
        {
            return new TextCaseInfo(
                "spell-component-substitution-candidate",
                "candidate",
                0.76,
                "Text appears to replace, supply, or waive spell components without granting a spell.",
                "SpellComponentSubstitutionParser");
        }

        if (SpellTemplateEffectRegex().IsMatch(cleaned))
        {
            return new TextCaseInfo(
                "spell-template-effect-candidate",
                "candidate",
                0.76,
                "Text uses an existing spell as a template for another item or feature effect.",
                "SpellTemplateEffectParser");
        }

        if (SpellCancellationEffectRegex().IsMatch(cleaned))
        {
            return new TextCaseInfo(
                "spell-cancellation-effect-candidate",
                "candidate",
                0.76,
                "Text cancels, absorbs, destroys, or tracks levels of incoming or active spells.",
                "SpellCancellationEffectParser");
        }

        if (SpellStorageEffectRegex().IsMatch(cleaned))
        {
            return new TextCaseInfo(
                "spell-storage-effect-candidate",
                "candidate",
                0.74,
                "Text appears to store, contain, copy, or preserve spells in an object or creature.",
                "SpellStorageEffectParser");
        }

        if (ItemAutonomousSpellEffectRegex().IsMatch(cleaned))
        {
            return new TextCaseInfo(
                "item-autonomous-spell-effect-candidate",
                "candidate",
                0.74,
                "Text describes an item or non-character object casting or emitting a spell effect.",
                "ItemAutonomousSpellEffectParser");
        }

        if (SpellDetectionReferenceRegex().IsMatch(cleaned))
        {
            return new TextCaseInfo(
                "spell-detection-reference-candidate",
                "candidate",
                0.72,
                "Text references a spell revealing, detecting, or identifying another property.",
                "SpellDetectionReferenceParser");
        }

        if (SpellRechargeRuleRegex().IsMatch(cleaned))
        {
            return new TextCaseInfo(
                "spell-recharge-rule-candidate",
                "candidate",
                0.72,
                "Text limits when an item or feature can cast the same spell again.",
                "SpellRechargeRuleParser");
        }

        if (SpellConsumableUseRegex().IsMatch(cleaned))
        {
            return new TextCaseInfo(
                "spell-consumable-use-candidate",
                "candidate",
                0.72,
                "Text describes a spell-bearing consumable being spent, faded, or destroyed after casting.",
                "SpellConsumableUseParser");
        }

        if (ItemRandomSpellEffectRegex().IsMatch(cleaned))
        {
            return new TextCaseInfo(
                "item-random-spell-effect-candidate",
                "candidate",
                0.74,
                "Text appears to be a random item table result that casts a spell.",
                "ItemRandomSpellEffectParser");
        }

        if (SpellSensorEffectRegex().IsMatch(cleaned))
        {
            return new TextCaseInfo(
                "spell-sensor-effect-candidate",
                "candidate",
                0.72,
                "Text modifies perception or targeting around a spell sensor.",
                "SpellSensorEffectParser");
        }
    ...
    [GeneratedRegex(@"^\s*Spell or item\s*$", RegexOptions.IgnoreCase | RegexOptions.CultureInvariant)]
    private static partial Regex SpellOrItemTableLabelRegex();

    [GeneratedRegex(@"^\s*(?:Enspelled (?:Armor|Staff|Weapon)|Spell Gem|Spell Scroll|Spellwrought Tattoo)(?:\s+\([^)]*\))?(?:\|[A-Za-z0-9-]+)?\s*$", RegexOptions.IgnoreCase | RegexOptions.CultureInvariant)]
    private static partial Regex ItemGroupMemberReferenceRegex();

    [GeneratedRegex(@"^\s*(?:\d+(?:st|nd|rd|th)\s+level|level\s+\d+|\d+/(?:day|month|7 days))\s*:\s*(?:\{@spell\s+[^}]+}\s*(?:,|and)?\s*)+$", RegexOptions.IgnoreCase | RegexOptions.CultureInvariant)]
    private static partial Regex SpellbookLevelListRegex();
    ...
    [GeneratedRegex(@"\b(?:when found, .* contains the following spells|book contains the following spells|following spells are recorded|spells are recorded in|contains the following spells, which are wizard spells)\b.*\{@spell\b", RegexOptions.IgnoreCase | RegexOptions.CultureInvariant)]
    private static partial Regex ItemSpellbookListSentenceRegex();
    ...
    [GeneratedRegex(@"\b(?:spells on the .* table are added to (?:the|that feature's) spell list|added to the spell list of your spellcasting class|add .* spells? to your spell list|add \{@spell [^}]+} to the list of spells)\b", RegexOptions.IgnoreCase | RegexOptions.CultureInvariant)]
    ...
    [GeneratedRegex(@"\b(?:substitute for|material components?|components you need to cast|without any somatic or verbal components|requires no spell components?|spell components are provided|provide the .* components)\b", RegexOptions.IgnoreCase | RegexOptions.CultureInvariant)]
    private static partial Regex SpellComponentSubstitutionRegex();

    [GeneratedRegex(@"\b(?:functions as|function as|as the \{@spell|as a \{@spell|as if you had cast|as if having cast|as if cast|as if with|as if using|as if created by|serves you as if created by|under the effect of|affected by a \{@spell|target of a \{@spell|equivalent to that of the \{@spell|comparable to that produced by a \{@spell|gains? the (?:effects?|benefit) of (?:the|a) \{@spell|gain the benefit of a \{@spell|no \{@status concentration} required|with all the attendant)\b", RegexOptions.IgnoreCase | RegexOptions.CultureInvariant)]
    private static partial Regex SpellTemplateEffectRegex();

    [GeneratedRegex(@"\b(?:cancel(?:s|ed)?|absorb(?:s|ed)?|destroy(?:s|ed|ing)?|dispel(?:s|led)?|counter(?:s|ed)?|suppress(?:es|ed|ing)?|levels of spells|spell whose level|active spells?)\b.*\b(?:spell|spells|magic|{@spell)\b|\b(?:spell|spells|magic|{@spell)\b.*\b(?:cancel(?:s|ed)?|absorb(?:s|ed)?|destroy(?:s|ed|ing)?|dispel(?:s|led)?|counter(?:s|ed)?|suppress(?:es|ed|ing)?|levels of spells|active spells?)\b", RegexOptions.IgnoreCase | RegexOptions.CultureInvariant)]
    private static partial Regex SpellCancellationEffectRegex();

    [GeneratedRegex(@"\b(?:store|stores|stored|contains?|hold|copy|replica|spell scroll|spellcasting magic|spell within|contains a crystallized bit of spellcasting magic|ring of spell storing|secret chest)\b.*\b(?:spell|spells|spell scroll|spellcasting magic|{@spell|{@item ring of spell storing)\b", RegexOptions.IgnoreCase | RegexOptions.CultureInvariant)]
    private static partial Regex SpellStorageEffectRegex();

    [GeneratedRegex(@"\b(?:vessel|dodecahedron|object|item|die|orb)\s+casts?\s+(?:the\s+)?\{@spell\b|\b(?:dodecahedron|object|item|die|lantern|statuette|figurine|orb)\b.*\bcasts?\s+(?:the\s+)?\{@spell\b|\b(?:emanate light equivalent to|sheds light comparable to|spell effects can't pass through|light equivalent to that of the \{@spell)\b", RegexOptions.IgnoreCase | RegexOptions.CultureInvariant)]
    private static partial Regex ItemAutonomousSpellEffectRegex();

    [GeneratedRegex(@"(?:\{@spell\s+(?:identify|detect evil and good)[^}]*}|identify spell|detect evil and good spell).*?\b(?:reveals?|detected|determine|learn|know|contains?|alignment|creature|property)\b|\b(?:reveals?|detected|determine|learn|know|alignment)\b.*?(?:\{@spell\s+(?:identify|detect evil and good)[^}]*}|identify spell|detect evil and good spell)", RegexOptions.IgnoreCase | RegexOptions.CultureInvariant)]
    private static partial Regex SpellDetectionReferenceRegex();

    [GeneratedRegex(@"\b(?:once .* used to cast a spell|once .* cast a spell|used to cast a spell|can't be used to cast that spell again|can't use .* cast that spell again|until the next dawn|until dawn|regains? .* ability to cast)\b", RegexOptions.IgnoreCase | RegexOptions.CultureInvariant)]
    private static partial Regex SpellRechargeRuleRegex();

    [GeneratedRegex(@"\b(?:once the spell is cast|spell is cast).*?\b(?:words .* fade|scroll .* crumbles|tattoo .* vanishes|consumed|destroyed)\b|\b(?:words .* fade|scroll .* crumbles|tattoo .* vanishes|consumed|destroyed)\b.*?\b(?:spell is cast|cast the spell)\b", RegexOptions.IgnoreCase | RegexOptions.CultureInvariant)]
    private static partial Regex SpellConsumableUseRegex();

    [GeneratedRegex(@"\b(?:you cast|instead, you cast|the vessel casts?|roll .* determine the spell)\s+(?:a spell originating from .*|\{@spell|the \{@spell)|\b(?:on a \{@b|\{@b [^}]+\.})[^.]*\b(?:casts?|cast)\s+(?:the\s+)?\{@spell\b", RegexOptions.IgnoreCase | RegexOptions.CultureInvariant)]
    private static partial Regex ItemRandomSpellEffectRegex();

    [GeneratedRegex(@"\b(?:spell's sensor|spell sensor|centered on the spell's sensor|while \{@spell scrying)\b", RegexOptions.IgnoreCase | RegexOptions.CultureInvariant)]
    private static partial Regex SpellSensorEffectRegex();

    [GeneratedRegex(@"\b(?:charges?|expend|while holding|while wearing|while attuned|as an action|bonus action|command word|study .* at the end of a long rest|cast .* from the item|use .* to cast|using .* to cast|use .* and cast|allows you to cast|granted the ability to cast|gain the ability to cast|cast \{@spell|casts? \{@spell|gain the effect of the \{@spell|from the .* and cast|pressing .* you cast|summoned as if you had cast)\b.*\b(?:spell|cantrip|spell slot|{@spell)\b|^\s*cast\s+\{@spell\b", RegexOptions.IgnoreCase | RegexOptions.CultureInvariant)]
    ...
    [GeneratedRegex(@"\b(?:expend|spend)\s+(?:\d+|one|a|an)\s+charges?\b.*\b(?:replace|prepare|prepared|swap)\b.*\b(?:spell|cantrip|wizard spells?|different spell)\b|\b(?:replace|prepare|prepared|swap)\b.*\b(?:spell|cantrip|wizard spells?|different spell)\b.*\b(?:expend|spend)\s+(?:\d+|one|a|an)\s+charges?\b", RegexOptions.IgnoreCase | RegexOptions.CultureInvariant)]
    private static partial Regex ItemChargeSpellPreparationRegex();

    [GeneratedRegex(@"\b(?:when you cast|whenever you cast)\b.*\b(?:spell|cantrip)\b.*\b(?:expend|spend)\s+(?:up to\s+)?(?:\d+|one|a|an)\s+(?:of its |the item's |the staff's |the orb's |charges? )?charges?\b|\b(?:expend|spend)\s+(?:up to\s+)?(?:\d+|one|a|an)\s+charges?\b.*\b(?:ignore .* components?|without .* components?|impose disadvantage|reroll|modify the spell)\b", RegexOptions.IgnoreCase | RegexOptions.CultureInvariant)]
    private static partial Regex ItemChargeSpellModifierRegex();

    [GeneratedRegex(@"\b(?:at will|at-will)\b.*\b(?:cast|casts|spell|cantrip|{@spell)\b|\b(?:cast|casts|{@spell)\b.*\b(?:at will|at-will)\b", RegexOptions.IgnoreCase | RegexOptions.CultureInvariant)]
    private static partial Regex ItemAtWillSpellActivationRegex();
    ...
    [GeneratedRegex(@"\b(?:gain the effect of|gains the effect of|duplicate(?:s| the)? effect of|as if you had cast|as if cast|as if with|summoned as if|functions as|causes? .* to emanate)\b.*\b(?:spell|cantrip|{@spell)\b", RegexOptions.IgnoreCase | RegexOptions.CultureInvariant)]
    private static partial Regex ItemSpellEffectActivationRegex();

    [GeneratedRegex(@"\b(?:gain|granted)\b.*\bability to cast\b.*(?:\{@spell|any spell).*?(?:\{@dice\s+\dd\d+}|\d+\s+times?|once)|(?:\{@spell|any spell).*?(?:\{@dice\s+\dd\d+}|\d+\s+times?)", RegexOptions.IgnoreCase | RegexOptions.CultureInvariant)]
    private static partial Regex ItemGrantedLimitedSpellUsesRegex();

    [GeneratedRegex(@"\b(?:loses charges? when|targeted by certain spells|comes into contact with certain spell|expends? \d+ of its charges? to cast|starts its turn .* expends? \d+ of its charges?)\b", RegexOptions.IgnoreCase | RegexOptions.CultureInvariant)]
    private static partial Regex ItemTriggeredChargeSpellEffectRegex();
    ...
    [GeneratedRegex(@"\b(?:use|take|as)\s+(?:your\s+)?(?:an?\s+)?(?:action|bonus action|\{@action Magic\|XPHB} action|Magic action)\b.*\b(?:one of the following spells|following spells|cast either|cast one of its spells|cast one of the spells)\b", RegexOptions.IgnoreCase | RegexOptions.CultureInvariant)]
    private static partial Regex ItemActionSpellListActivationRegex();

    [GeneratedRegex(@"\b(?:use|take|as)\s+(?:your\s+)?(?:an?\s+)?(?:action|bonus action|\{@action Magic\|XPHB} action|Magic action)\b.*\b(?:use .* to cast|cast (?:the\s+)?\{@spell|cast \{@spell|to cast \{@spell|to cast (?:the\s+)?\{@spell)\b|\b(?:as an action|as a bonus action|as a \{@action Magic\|XPHB} action)\b.*\b(?:cast|{@spell)\b", RegexOptions.IgnoreCase | RegexOptions.CultureInvariant)]
    private static partial Regex ItemActionDirectSpellActivationRegex();

    [GeneratedRegex(@"\b(?:allows? you to cast|can use it to cast|can be used to cast|can use .* to cast|while .* you can cast|you can cast \{@spell|you can cast one of the following spells|can cast \{@spell|can cast one of the following spells)\b", RegexOptions.IgnoreCase | RegexOptions.CultureInvariant)]
    private static partial Regex ItemPassiveDirectSpellAccessRegex();
    ...
    [GeneratedRegex(@"\b(?:allows you to cast|allow you to cast|can use it to cast|can use .* to cast|can be used to cast|used to cast a given spell|play .* to cast|you can cast \{@spell|you may cast \{@spell|you can cast one of the following spells|knows one of the following spells and can cast it at will)\b", RegexOptions.IgnoreCase | RegexOptions.CultureInvariant)]
    ...
    [GeneratedRegex(@"\b(?:removed with|remove(?:d)? by|ended with|ended only with|spell can end|spell ends|ends the \{@spell|undone by|can be undone|broken only by|can be broken only by|dispel magic|remove curse|greater restoration|lesser restoration|similar magic)\b.*\b(?:spell|magic|curse|effect|condition|{@spell)\b|\b\{@spell (?:remove curse|greater restoration|lesser restoration|dispel magic|wish|antimagic field)[^}]*}\b.*\b(?:undone|broken|ends?|except|suppress(?:es)?)\b", RegexOptions.IgnoreCase | RegexOptions.CultureInvariant)]
    ...
    [GeneratedRegex(@"\b(?:fails? a saving throw against a spell|saving throw against .* spell|damage .* from .* spell|spell that deals damage|attack, spell, or feature|from that spell|one target of the spell|target of the spell|targeting only|spell cast .* causes|spell cast .* gives|spell-created|can't cast spells|cursed with \{@spell|takes? .* damage from your .* spell)\b", RegexOptions.IgnoreCase | RegexOptions.CultureInvariant)]
